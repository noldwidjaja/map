...
(function() {
    ...
    var RECIPES = {
        "temp": {min: -10,   max: 35,    scale: "line", precision: 1, label: "æ°—æ¸© Temperature", unit: "ÂºC"},
        "hum":  {min: 0,     max: 100,   scale: "line", precision: 1, label: "æ¹¿åº¦ Humidity", unit: "%"},
        "wv":   {min: 1,     max: 20,    scale: "log",  precision: 1, label: "é¢¨é€Ÿ Wind Velocity", unit: " m/s"},
        "in":   {min: 0.1,   max: 4.0,   scale: "log",  precision: 2, label: "æ—¥å°„é‡ Insolation", unit: ' MJ/m<span class="sup">2</span>'},
        "no":   {min: 0.001, max: 0.600, scale: "log",  precision: 0, label: "ä¸€é…¸åŒ–çª’ç´  Nitric Monoxide", unit: " ppb", multiplier: 1000},
        "no2":  {min: 0.001, max: 0.200, scale: "log",  precision: 0, label: "äºŒé…¸åŒ–çª’ç´  Nitrogen Dioxide", unit: " ppb", multiplier: 1000},
        "nox":  {min: 0.001, max: 0.600, scale: "log",  precision: 0, label: "çª’ç´ é…¸åŒ–ç‰© Nitrogen Oxides", unit: " ppb", multiplier: 1000},
        "ox":   {min: 0.001, max: 0.250, scale: "log",  precision: 0, label: "å…‰åŒ–å­¦ã‚ªã‚­ã‚·ãƒ€ãƒ³ãƒˆ Photochemical Oxidants", unit: " ppb", multiplier: 1000},
        "so2":  {min: 0.001, max: 0.110, scale: "log",  precision: 0, label: "äºŒé…¸åŒ–ç¡«é»„ Sulfur Dioxide", unit: " ppb", multiplier: 1000},
        "co":   {min: 0.1,   max: 3.0,   scale: "log",  precision: 1, label: "ä¸€é…¸åŒ–ç‚­ç´  Carbon Monoxide", unit: " ppm"},
        "ch4":  {min: 1.5,   max: 3.0,   scale: "log",  precision: 2, label: "ãƒ¡ã‚¿ãƒ³ Methane", unit: " ppm"},
        "nmhc": {min: 0.01,  max: 1.30,  scale: "log",  precision: 2, label: "éžãƒ¡ã‚¿ãƒ³ç‚­åŒ–æ°´ç´  Non-Methane Hydrocarbons", unit: " ppm"},
        "spm":  {min: 1,     max: 750,   scale: "log",  precision: 0, label: "æµ®éŠç²’å­çŠ¶ç‰©è³ª Suspended Particulate Matter", unit: ' Î¼g/m<span class="sup">3</span>'},
        "pm25": {min: 1,     max: 750,   scale: "log",  precision: 0, label: "å¾®å°ç²’å­çŠ¶ç‰©è³ª 2.5Âµm Particulate Matter", unit: ' Î¼g/m<span class="sup">3</span>'}
    };
    ...
    function parseDisplayData() {
        ...
        return {
            topography: "/data/tokyo-topo.json",
    ...
    function displayStatus(status, error) {
        if (error) {
            ...
            d3.select(STATUS_ID).node().textContent = "â‚ " + error;
        }
        else if (!bad) {
            d3.select(STATUS_ID).node().textContent = "â‚ " + status;
        }
    }
    ...
    function componentize(wind) {
        var Ï† = wind[0] / 360 * Ï„;  // meteorological wind direction in radians
        var m = wind[1];  // wind velocity, m/s
        var u = -m * Math.sin(Ï†);  // u component, zonal velocity
        var v = -m * Math.cos(Ï†);  // v component, meridional velocity
    ...
    function formatCoordinates(lng, lat) {
        return Math.abs(lat).toFixed(6) + "Âº " + (lat >= 0 ? "N" : "S") + ", " +
            Math.abs(lng).toFixed(6) + "Âº " + (lng >= 0 ? "E" : "W");
    }
    ...
    function formatVector(x, y) {
        var d = Math.atan2(-x, y) / Ï„ * 360;  // calculate into-the-wind cardinal degrees
        var wd = Math.round((d + 360) % 360 / 5) * 5;  // shift [-180, 180] to [0, 360], and round to nearest 5.
        var m = Math.sqrt(x * x + y * y);
        return wd.toFixed(0) + "Âº @ " + m.toFixed(1) + " m/s";
    }
    ...
    function interpolateField(stations, data, settings, masks) {
        ...
        if (points.length < 5) {
            return d.reject("æ±äº¬éƒ½ç’°å¢ƒå±€ãŒãƒ‡ãƒ¼ã‚¿ã‚’èª¿æ•´ä¸­");
        }
    ...
    function postInit(settings, field, overlay) {
        ...
        d3.select(DISPLAY_ID).on("click", function() {
            ...
            var v = field(p[0], p[1]);
            if (v[2] >= INVISIBLE) {
                d3.select(LOCATION_ID).text("â‚ " + formatCoordinates(c[0], c[1]));
                var pointDetails = "â‚ " + formatVector(v[0], v[1]);
                if (overlay) {
